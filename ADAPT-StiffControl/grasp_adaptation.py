...
stiffness_descent: K_task for the four fingers (index/middle/ring/pinky) updated
each tick to drive analytic tip force → f_des = F_GAIN / C_O.

Protocol:
  1. UR5 → GRASP_POSE; settle SETTLE_TIME s; hand ramps HOME → PC1 at K_TIP_GENTLE
  2. Record pos_gentle, F_gentle for probe fingers for SENSE_DURATION s (baseline)
  3. Freeze thumb at K_TIP_HOLD; ramp probe fingers K → K_TIP_PROBE; wait convergence
  4. Record pos_probe, F_probe for SENSE_DURATION s
  5. C_O = mean over fingers of ||Δpos|| / ||ΔF||; f_des = F_GAIN / C_O
  6. Gradient descent on K_task per finger until mean |f_meas − f_des| < F_CONVERGE_THR
  7. UR5 presses −PRESS_HEIGHT (shows exerted force)
  8. Release hand (k=0); arm stays at PRESS_POSE
"""
...
PROBE_FINGERS = ['index', 'middle', 'ring', 'pinky']
...
# Per-probe-finger GD initial K_joint (flex passive → zeros)
_GD_K_JOINT_INIT = {f: np.zeros((3, 3)) for f in PROBE_FINGERS}
...
def _csv_header():
    cols = ['time_s', 'phase', 'C_O_m_per_N',
            'f_des_mean_x_N', 'f_des_mean_y_N', 'f_des_mean_z_N', 'f_des_mean_mag_N',
            'f_meas_gd_mean_x_N', 'f_meas_gd_mean_y_N', 'f_meas_gd_mean_z_N', 'f_meas_gd_mean_mag_N',
            'converged']
    # GD probe-finger mean task stiffness (diagonal, 3 values)
    for ax in range(3):
        cols.append(f'K_probe_task_mean_{ax}{ax}_Npm')
    # GD probe-finger mean joint stiffness (diagonal, 3 values — flex joints)
    for i in range(3):
        cols.append(f'K_probe_joint_mean_{i}_Nmrad')
    # GD probe-finger mean ref position
    for ax in 'xyz':
        cols.append(f'd_ref_probe_mean_{ax}_m')
    # GD probe-finger mean joint ref (degrees, 3 flex joints)
    for i in range(3):
        cols.append(f'theta_ref_probe_mean_{i}_deg')
...
def _probe_K_task(k_probe):
    """K_task dict: probe fingers at k_probe, thumb clamped at K_TIP_HOLD (or k_probe before freeze)."""
    k_thumb = K_TIP_HOLD if _thumb_hold_pos else k_probe
    K = {f: (k_probe if f != 'thumb' else k_thumb) * np.eye(3) for f in FINGERTIPS}
    K['palm'] = k_probe * np.eye(3)
    return K


def _compute_row(q, q_dot, phase, C_O, f_des_mean, f_meas_mean,
                 gd_K_task_diag_mean, gd_K_joint_diag_mean,
                 gd_d_ref_mean, gd_theta_ref_mean,
                 converged):
    K_task_now = _probe_K_task(_current_k)

    row = [f'{time.time() - _experiment_start:.4f}', phase,
           f'{C_O:.6e}']
    row += [f'{v:.6f}' for v in f_des_mean]
    row.append(f'{float(np.linalg.norm(f_des_mean)):.6f}')
    row += [f'{v:.6f}' for v in f_meas_mean]
    row.append(f'{float(np.linalg.norm(f_meas_mean)):.6f}')
    row.append(int(converged))

    # GD probe mean task stiffness diagonal (3)
    for v in gd_K_task_diag_mean:
        row.append(f'{v:.6f}')
    # GD probe mean joint stiffness diagonal (3)
    for v in gd_K_joint_diag_mean:
        row.append(f'{v:.6f}')
    # GD probe mean d_ref (3)
    row += [f'{v:.6f}' for v in gd_d_ref_mean]
    # GD probe mean theta_ref (3)
    row += [f'{v:.6f}' for v in gd_theta_ref_mean]
...
STATE_SENSE_REC       = 4   # record pos_gentle, F_gentle for probe fingers
STATE_PROBE_RAMP      = 5   # freeze thumb; ramp probe fingers K → K_TIP_PROBE
STATE_PROBE_CONV      = 6   # wait convergence at K_TIP_PROBE
STATE_PROBE_REC       = 7   # record probe; compute mean C_O; init GD per finger
STATE_ADAPT_GD        = 8   # gradient descent on all probe fingers until converged
STATE_PRESS           = 9   # UR5 pressing down; hold at PRESS_POSE
STATE_RELEASE         = 10  # k=0 at PRESS_POSE; wait HOLD_TIME
STATE_RAMP_HOME       = 11  # snap joint targets, ramp to HOME
STATE_RETRACT         = 12  # hand ramping to HOME; arm stays at PRESS_POSE
STATE_DONE            = 13
...
# Gradient-descent state (initialised in STATE_PROBE_REC, one entry per probe finger)
_gd_K_joint = {f: v.copy() for f, v in _GD_K_JOINT_INIT.items()}
_gd_K_task  = {f: K_TIP_GENTLE * np.eye(3) for f in PROBE_FINGERS}
_gd_d_ref   = {f: D_REF[f].copy() for f in PROBE_FINGERS}
_gd_f_des   = {f: np.zeros(3) for f in PROBE_FINGERS}
_gd_f_meas  = {f: np.zeros(3) for f in PROBE_FINGERS}
...
def _freeze_and_hold_thumb(q):
    """Clamp thumb at current position with K_TIP_HOLD."""
    pos = _tip_pos('thumb', q)
    _thumb_hold_pos['thumb']              = pos.copy()
    vmc_task.targets['thumb']             = pos.copy()
    vmc_task.springs['thumb'].stiffness   = np.full(3, K_TIP_HOLD)
    _d_ref_model_dict['thumb']            = pos.copy()
...
def _begin_ramp(end_targets):
    ...
    _ramp_start_targets = {
        ...
        'spread':            {f: vmc_joint.spread[f].copy() for f in PROBE_FINGERS},
...
def _step_k_ramp(now):
    """Ramp K for all probe fingers simultaneously."""
    global _current_k
    alpha      = min(1.0, (now - _k_ramp_t0) / RAMP_DURATION)
    _current_k = (1 - alpha) * _k_ramp_start + alpha * _k_ramp_end
    for _f in PROBE_FINGERS:
        vmc_task.springs[_f].stiffness = np.full(3, _current_k)
    return alpha >= 1.0


def _gd_log_extras():
    """Mean GD state across probe fingers for CSV logging."""
    K_task_diag_mean  = np.mean([np.diag(_gd_K_task[f])  for f in PROBE_FINGERS], axis=0)
    K_joint_diag_mean = np.mean([np.diag(_gd_K_joint[f]) for f in PROBE_FINGERS], axis=0)
    d_ref_mean        = np.mean([_gd_d_ref[f]            for f in PROBE_FINGERS], axis=0)
    # mean theta_ref from THETA_REF_DEG (finger flex joints: 3 per finger)
    theta_ref_mean = np.mean([
        THETA_REF_DEG[8 + i * 3: 8 + i * 3 + 3]
        for i in range(len(PROBE_FINGERS))
    ], axis=0)
    f_des_mean  = np.mean([_gd_f_des[f]  for f in PROBE_FINGERS], axis=0)
    f_meas_mean = np.mean([_gd_f_meas[f] for f in PROBE_FINGERS], axis=0)
    return (K_task_diag_mean, K_joint_diag_mean,
            d_ref_mean, theta_ref_mean,
            f_des_mean, f_meas_mean)
...
def control_callback():
    ...
    if state == STATE_APPROACH:
        ...
        if _log_tick % LOG_EVERY == 0:
            K_task_now = _probe_K_task(K_TIP_GENTLE)
            for _f in PROBE_FINGERS:
            ...
            if not COLLECTED_DATA:
                ...
                _csv_writer.writerow(_compute_row(
                    q, q_dot, 'sense', 0.0,
                    extras[4], extras[5],
                    *extras[:4], converged=True))
        if elapsed >= SENSE_DURATION:
            if not COLLECTED_DATA:
                _csv_file.flush()
            _freeze_and_hold_thumb(q)
            controller.get_logger().info(
                f'Gentle point recorded ({_sense_count} samples). '
                f'Thumb frozen. Ramping probe fingers K {K_TIP_GENTLE} → {K_TIP_PROBE} N/m …')
        ...
        if _step_k_ramp(now):
            controller.get_logger().info(
                f'Probe fingers at K={K_TIP_PROBE} N/m. Waiting convergence …')
            _converge_ticks = 0
            _converged      = False
            _log_tick       = 0
            _state_start    = now
        ...
        if elapsed >= SENSE_DURATION:
            ...
            # Compute compliance per probe finger, then average
            ...
            _C_O_mean = float(np.mean(C_O_list)) if C_O_list else 0.0
            ...
            for _f in PROBE_FINGERS:
                f_init = np.asarray(stiff_model.tip_force(
                    _f, q, THETA_REF_DEG, _d_ref_model_dict,
                    {_f: _GD_K_JOINT_INIT[_f]}, {_f: K_TIP_PROBE * np.eye(3)}))
                f_init_mag = float(np.linalg.norm(f_init))
                if f_init_mag > 1e-9:
                    _gd_f_des[_f] = f_des_mag * f_init / f_init_mag
                else:
                    _gd_f_des[_f] = np.array([0.0, 0.0, -f_des_mag])
                _gd_K_joint[_f] = np.zeros((3, 3))
                _gd_K_task[_f]  = K_TIP_PROBE * np.eye(3)
                _gd_d_ref[_f]   = _d_ref_model_dict[_f].copy()

            controller.get_logger().info(
                f'Probe done ({len(C_O_list)}/{len(PROBE_FINGERS)} fingers valid). '
                f'C_O = {_C_O_mean * 1e3:.2f} mm/N → f_des = {f_des_mag:.3f} N')
            _gd_converge_ticks = 0
            _log_tick          = 0
            _state_start       = now
        ...
        # Per-finger force measurement and GD step
        f_errs = []
        for _f in PROBE_FINGERS:
            ...
            k_task_diag = np.diag(_gd_K_task[_f])
            vmc_task.springs[_f].stiffness = k_task_diag
            f_errs.append(float(np.linalg.norm(_gd_f_meas[_f] - _gd_f_des[_f])))

        _current_k = float(np.mean([np.mean(np.diag(_gd_K_task[f])) for f in PROBE_FINGERS]))

        f_err = float(np.mean(f_errs))
        ...
        if not COLLECTED_DATA and _log_tick % LOG_EVERY == 0:
            ...
            _csv_writer.writerow(_compute_row(
                ...
                extras[4], extras[5],
                *extras[:4], converged=gd_converged))
        ...
        if gd_converged or elapsed >= CONVERGE_TIMEOUT:
            ...
            controller.get_logger().info(
                ...
                f'mean |f_err| = {f_err:.4f} N. Pressing down …')
            _log_tick    = 0
            _state_start = now
...
controller.get_logger().info(
    f'Grasp adaptation | object: {OBJECT_NAME} | probe fingers: {PROBE_FINGERS} | '
