...
## ==================================
## Joint-space spring-dampers for the 13 torque-controlled motors.
## Stiffness and damping are configurable per joint group via self.stiffness / self.damping.

import numpy as np
from KinematicsHand.FK_Hand import FK_motor2thumb, FK_motor2finger, FK_motor2spread
...
class VMC:
    """
    Joint-space spring-damper controller for 20 hand DOFs (wrist excluded).

    Stiffness and damping dicts are keyed by joint group:
      thumb(4) + spread×4(4) + index(3) + middle(3) + ring(3) + pinky(3) = 20 DOF
    """
    ...
    def hand_torques(self, q_motor, q_dot_motor):
        ...
        # Spread (one DOF per finger, shared spread motor)
        ...
        # Fingers
        for finger, target in [('index',  self.index_target),
                                ('middle', self.middle_target),
                                ('ring',   self.ring_target),
                                ('pinky',  self.pinky_target)]:
