"""
Supplementary — softening on contact (finger).

Inverse of TunableCompliance/stiffening_contact.py. The finger starts stiff
(K_max) and K_d is reduced online each control cycle via the exponential
softening law:

    K(F) = K_min + (K_max - K_min) · exp(−alpha · F)

where F is the measured normal contact force [N].

  - alpha = 0       : K = K_max throughout  (no softening - stiff baseline)
  - alpha = small   : gradual softening  (F_half = ln(2)/alpha)
  - alpha = large   : near-immediate softening at first contact

Alpha values in ALPHA_SWEEP use the same criterion as stiffening_contact.py
(F_half = ln(2)/alpha spans the expected contact-force range logarithmically)
for direct comparison.

Protocol (per run):
  1. UR5 moves to UR5_POSE; finger lifted to FINGER_STRAIGHT.
  2. Wait SETTLE_TIME for finger to lift.
  3. Set run alpha, initialise K_d to K_max, set target FINGER_TARGET;
      wait SETTLE_TIME.
...
Outputs:
  Supplementary/outputs/softening_contact/alpha_X/alpha_X_run_N.csv
"""
...
K_MIN = 0.10    # [N·m/rad] stiffness at saturation (large force)
K_MAX = 0.60    # [N·m/rad] stiffness at zero contact force

# alpha [1/N]: F_half = ln(2)/alpha where K reaches midpoint of [K_min, K_max]
...
def softening_k(alpha, force):
    """K(F) = K_min + (K_max - K_min) * exp(-alpha * F)."""
    if alpha == 0:
        return K_MAX
    return K_MIN + (K_MAX - K_MIN) * np.exp(-alpha * max(force, 0.0))

def set_experiment_stiffness():
    """Initialise spring to K_max before descent (same for all alphas)."""
    vmc.spring = LinearSpring(np.array([K_MAX] * 3))

def output_path(alpha, run):
    folder = os.path.join(
        os.path.dirname(__file__), 'outputs', 'softening_contact', f'alpha_{alpha}')
...
def control_callback():
    ...
    if state in (STATE_DESCEND, STATE_ASCEND):
        k = softening_k(current_alpha, controller.force_N)
...
controller.get_logger().info(
    f'Softening contact: {len(ALPHA_SWEEP)} alpha values × {N_RUNS} runs = '
