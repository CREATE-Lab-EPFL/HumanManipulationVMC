...
def _csv_header():
    cols = ['time_s', 'phase', 'k_Nmrad', 'converged']
    for i in range(13):
...
def _compute_row(q, q_dot, phase, k, converged):
    row = [f'{time.time() - _experiment_start:.4f}', phase, f'{k:.4f}', int(converged)]
...
def _reset_trial():
    ...
    global _converge_ticks, _converged, _sweep_idx, _current_k
...
def _begin_k_ramp(k_start, k_end, after_state):
...
def control_callback():
    ...
    if state == STATE_INIT_ARM:
        ...
        if _step_ramp(now):
            ...
            controller.get_logger().info(
                f'Ramp done. Flexion springs at K = {K_GENTLE} N·m/rad …')
        ...
        if not COLLECTED_DATA and _log_tick % LOG_EVERY == 0:
            _csv_writer.writerow(_compute_row(q, q_dot, 'gentle', K_GENTLE, True))
        if elapsed >= RECORD_DURATION:
            if not COLLECTED_DATA:
                _csv_file.flush()
            _sweep_idx   = 0
            _state_start = now
            _begin_k_ramp(K_GENTLE, K_SWEEP[0], STATE_SWEEP_CONV)
            state = STATE_K_RAMP
            controller.get_logger().info(
                f'Baseline recorded. Ramping K {K_GENTLE:.3f} → '
                f'{K_SWEEP[0]} N·m/rad over {RAMP_DURATION:.1f} s '
                f'(sweep step 1/{len(K_SWEEP)})…')

    elif state == STATE_K_RAMP:
        ...
        if (_converge_ticks >= _CONVERGE_TICKS) or (elapsed >= CONVERGE_TIMEOUT):
            if not _converged:
                ...
                controller.get_logger().info(
                    f'K = {_current_k:.3f} N·m/rad converged at {elapsed:.1f} s. '
                    f'Recording {RECORD_DURATION:.0f} s …')

    elif state == STATE_SWEEP_REC:
        _log_tick += 1
        if not COLLECTED_DATA and _log_tick % LOG_EVERY == 0:
            _csv_writer.writerow(_compute_row(q, q_dot, 'sweep', _current_k, True))
        if elapsed >= RECORD_DURATION:
            if not COLLECTED_DATA:
                _csv_file.flush()
            _sweep_idx += 1
            if _sweep_idx >= len(K_SWEEP):
                controller.get_logger().info(
                    'Sweep complete. Releasing flexion springs before returning home …')
                _set_flex_stiffness(K_ROT)
                _state_start = now
                state        = STATE_UNLOAD
            else:
                k_prev = _current_k
                k_next = K_SWEEP[_sweep_idx]
                _state_start = now
                _begin_k_ramp(k_prev, k_next, STATE_SWEEP_CONV)
                state = STATE_K_RAMP
                controller.get_logger().info(
                    f'Ramping K {k_prev:.3f} → {k_next} N·m/rad over '
                    f'{RAMP_DURATION:.1f} s (sweep step '
                    f'{_sweep_idx + 1}/{len(K_SWEEP)})…')

    elif state == STATE_UNLOAD:
        if elapsed >= CONVERGE_HOLD:
            controller.get_logger().info(
                f'Springs released. Ramping PC1 → HOME over {RAMP_DURATION:.1f} s …')
